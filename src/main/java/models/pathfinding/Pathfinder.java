...
public class Pathfinder {
     ...
     * than a single target. Calling this with {@code isDijkstra = false} throws,
     * because A* requires a target node to compute its heuristic.
     *
     * @param source      the source node; must not be {@code null}
     * @param isDijkstra  must be {@code true}; passing {@code false} throws
     *                    {@link RuntimeException}
     * @return a {@link Result} containing distances, back-pointers, and the
     *         visited set for the full Dijkstra run
     * @throws RuntimeException if {@code isDijkstra} is {@code false}
     */
    public Result _shortestPath(Node source, boolean isDijkstra) {
        if (!isDijkstra) {
            throw new RuntimeException("Must pass source and endNode, if you want to use A* for pathfinding");
        }
        return _shortestPath(source, null, true);
    }

    /**
     * Runs Dijkstra or A* from {@code source} toward {@code target}.
     *
     * <p>When {@code isDijkstra} is {@code true} the heuristic is fixed at zero,
     * making the algorithm equivalent to standard Dijkstra. When {@code false},
     * a haversine distance heuristic is used (A*), and the search exits as soon
     * as {@code target} is settled, which is typically much faster on geographic
     * graphs.
     *
     * <p>All nodes that are {@link Node#isSubmerged()} are
     * skipped unconditionally.
     *
     * @param source      the source node; must not be {@code null}
     * @param target     the destination node; must not be {@code null}
     * @param isDijkstra {@code true} to run Dijkstra, {@code false} to run A*
     * @return a {@link Result} containing distances, back-pointers, and the
     ...
     *                                  {@code null}
     */
    public Result _shortestPath(Node source, Node target, boolean isDijkstra) {
     ...
     *
     * <p>Internally calls {@link #_shortestPath(Node, Node, boolean)} with
     * {@code isDijkstra = false} and reconstructs the path by following the
    ...
    public Path getShortestPathTo(Node source, Node target) {
        Result result = _shortestPath(source, target, false);
