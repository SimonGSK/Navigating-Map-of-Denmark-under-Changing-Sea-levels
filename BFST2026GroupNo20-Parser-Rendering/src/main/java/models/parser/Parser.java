...
public class Parser implements IParser {
        ...
        try {
            ...
            while ((line = br.readLine()) != null){
                if(line.contains("<bounds")) {
                    ...
                    if (isFerryRelation(tags)) {
                        removeMemberWays(members);
                    }
                }
            }
    ...
    private boolean isFerryWay(HashMap<String, String> tags) {
        return "ferry".equals(tags.get("route"))
                || "ferry".equals(tags.get("disused:route"))
                || "ferry_route".equals(tags.get("seamark:type"));
    }

    private boolean isFerryRelation(HashMap<String, String> tags) {
        return "ferry".equals(tags.get("route"))
                || "route".equals(tags.get("type")) && tags.containsKey("seamark:ferry_route:category")
                || "ferry_route".equals(tags.get("seamark:type"));
    }

    private void removeMemberWays(List<Member> members) {
        for (Member member : members) {
            if (member.getElement() instanceof Way way) {
